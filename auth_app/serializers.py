...
class DonorRegisterSerializer(serializers.Serializer):
    ...
    def create(self, validated_data):
        ...
        # Update donor profile with full_name if provided
        if full_name and hasattr(user, 'donor_profile'):
            user.donor_profile.full_name = full_name
...
class DonorProfileSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = DonorProfile
        fields = [
            'id', 'email', 'photo', 'full_name', 'short_bio', 
    ...
    def validate_photo(self, value):
        if value:
            ...
            if value.size > 5 * 1024 * 1024:
                from .exceptions import FileSizeTooLargeException
